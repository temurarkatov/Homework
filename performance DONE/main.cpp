...
{
	//int (*function)() - óêàçàòåëü íà ôóíêöèþ, êîòîðàÿ íè÷åãî íå ïðèíèìàåò, è âîçâðàùàåò çíà÷åíèå òèïà 'int'.
	clock_t start = clock();
	T result = (tree.*function)();
	clock_t end = clock();
	cout << message << result << ", âû÷èñëåíî çà " << double(end - start) / CLOCKS_PER_SEC << " ñåêóíä\n";
}
...
void main()
{
	...
	tree.erase(75);*/
	tree.print();
	cout << "Ãðóáèíà äåðåâà: " << tree.depth() << endl;
#endif // ERASE_CHECK
	...
	measure_performance("Ìèíèìàëüíîå çíà÷åíèå â äåðåâå: ", &Tree::minValue, tree);
	measure_performance("Ìàêñèìàëüíîå çíà÷åíèå â äåðåâå: ", &Tree::maxValue, tree);
	measure_performance("Ñóììà ýëåìíòîâ äåðåâà: ", &Tree::sum, tree);
	measure_performance("Êîëè÷åñòâî ýëåìíòîâ äåðåâà: ", &Tree::count, tree);
	measure_performance("Ñðåäíåå-àðèôìåòè÷åñêî ýëåìíòîâ äåðåâà: ", &Tree::avg, tree);
	measure_performance("Ãëóáèíà äåðåâà: ", &Tree::depth, tree);
